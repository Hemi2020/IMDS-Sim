...
/**
 * ═══════════════════════════════════════════════════════════════════════
 * HH-60W IMDS TRAINING SIMULATOR - Improved Layout Version
 * ═══════════════════════════════════════════════════════════════════════
 * 
 * IMPROVEMENTS:
 * - Exact Screen 073 layout matching real IMDS screenshot
 * - Fixed textarea - can type continuously without clicking
 * - Consistent beige colors across all screens
 * - Julian date format (YYDDD) + military time maintained
 * - Future scheduling prevention maintained
...
const HH60W_WUC_CODES = [
  ...
  { code: "640000", desc: "Main Transmission" },
...
export default function IMDSTrainerImproved() {
      ...
      <div className="space-y-3">
        ...
        {f122.query && (
          <div className="bg-yellow-100 border-2 border-black p-4 rounded font-mono text-sm text-black">
            <div className="font-bold text-center mb-4 text-black">MAINTENANCE SNAPSHOT REPORT</div>
            
            {results.length === 0 ? (
              <div className="text-center py-4 text-black">No matching jobs found.</div>
            ) : (
              <div className="space-y-3">
                {results.map(j => (
                  <div key={j.jcn + j.wce} className="border border-black p-2 bg-yellow-50">
                    <div className="grid grid-cols-3 gap-2 mb-2 text-xs text-black">
                      ...
                      <div><strong>Status:</strong> <span className={
                        ...
                        j.status === 'IN-WORK' ? 'text-orange-700 font-bold' :
                    ...
                    <div className="text-xs mt-2 text-black">
                      <strong>Discrepancy:</strong> {j.discrepancy}
                    </div>
                    <div className="text-xs mt-1 text-gray-600">
                      <strong>Date/Time:</strong> {j.startDate} {j.startTime}
                    </div>
                  </div>
                ))}
              </div>
  ...
  function submit914() {
    if (!f914.jcn) {
      showMessage("JCN is required", "error");
      return;
    }
    
    const jobIdx = jobs.findIndex(j => j.jcn === f914.jcn.toUpperCase() && j.wce === f914.wce);
    if (jobIdx === -1) {
      showMessage(`JCN ${f914.jcn} / WCE ${f914.wce} not found`, "error");
      return;
    }
    ...
    setJobs(updated);
    showMessage(`✅ JCN ${f914.jcn} / WCE ${f914.wce} completed`, "success");
    ...
    <div className="space-y-3">
      ...
      <div className="bg-[#D4C5B0] border-2 border-black p-3 rounded">
          ...
          <div>
            ...
            >
              ...
              {jobs.filter(j => j.status !== 'COMPLETED').map(j => (
                <option key={j.jcn} value={j.jcn}>{j.jcn}</option>
              ))}
            </select>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            <label className="text-xs font-bold block mb-1">Action Taken Code</label>
            <select
          ...
          <div>
            <label className="text-xs font-bold block mb-1">How Malfunction Code</label>
            <select
  ...
  function exportCSV() {
    ...
    const headers = "JCN,WCE,Equipment,WUC,Work Center,Status,Priority,Discrepancy\n";
    const rows = filtered.map(j => 
      `${j.jcn},${j.wce},${j.equipmentId},${j.wuc},${j.workCenter},${j.status},${j.priority},"${j.discrepancy}"`
    ).join("\n");
